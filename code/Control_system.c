...
#define SAFE 2
#define MAX_TASKS 8
#define BUFFER_SIZE 40   // Enough to get as many bits sent during two calls of the reading function
#define HB 20   // Heartbeat = 20ms (minimum value among all period of the tasks executed in the scheduler)

// Heartbeat structure //
typedef struct{
    int n;  // Internal incrementation
    int N;  // number of heartbeat you need between two executions of the task
}heartbeat;
...
int rpm1 = 0;   // rpm for motor 1
int rpm2 = 0;   // rpm for motor 2
...
char *ACK_msg_type;     // Pointer to the string associated to the msg_type inside MCACK message 
int ACK_value;          // value of ACK: 1-->successful operation & 0-->unsuccessful operation
int ACK_enable=0;       // Allow to disable(0) or enable(1)the sending of MCACK messsage to the PC
...
heartbeat t3 = {.n = 0, .N = 20/HB};        // Every 20 ms (check_button())
...
heartbeat t8 = {.n = -2, .N = 200/HB};      // Every 200 ms (send_msg())
...
int main(void) { 
    ...
    T2CONbits.TON = 1;  // starts of tmr2 for TIMEOUT mode 
    
    // Initialization the tasks for the scheduler
    schedInfo[1] = t1;      
    schedInfo[2] = t2;      
    schedInfo[3] = t3;       
    schedInfo[4] = t4;      
    ...
    T1CONbits.TON = 1;  // Starts of tmr1 for the hearbeat of the scheduler 
...
void init_ADC(){
    ...
    ADCON2bits.SMPI = 0000; // IF high after one conversion
...
void init_PWM(){
    ...
    PWMCON1bits.PEN1H = 1;  // PWM1H pin is enabled for PWM output (motor 1)
    PWMCON1bits.PEN2H = 1;  // PWM2H pin is enabled for PWM output (motor 2)
...
} // Initialize the PWM with the corresponding pwm frequency
...
void init_UART(){
    ...
    U2STAbits.URXISEL = 0;  // Interrupt flag is set when a character is received 
    IEC0bits.T2IE = 1;  //Enable interrupt -> tmr2
    IEC1bits.U2RXIE = 1; //Enable interupt -> UART
} //setup the UART2 at 9600 baudrate
...
}// function to be able to stay synchronized with a the event of periodicity equal to the input variable ms
void tmr_wait_period(int timer) {
...
void LCD_display(){
    ...
    if (display == 0){
        ...
        // temperature //
        sprintf (strbuf, "%3.1f", (double)averTemp);   // Float with 3 digits before the coma and 1 after
        LCD_write(" temp:");
        LCD_write(strbuf);
        // applied rmp //
        move_cursor_middle();
...
}   // blink the Leds accordingly to the state we are in

// BUTTONS //
void check_button(){
    // If S6 is being pushed
...
// TEMPERATURE //
float get_SumTemperature(){
    // As the converion is automatic, we don't have to check if there is something to read
...
void send_MCACK(){
    if(ACK_enable == 1){
        ...
        ACK_enable = 0;     // The ACK feedback will not be send till needed
    }
}
...
int sat_requirements_check(int min, int max){
    ...
    if(min < -8000 || max > 8000 || min > max || min > 0 || max < 0){
        return(0);      // Received values are discarged
    } 
    // all the previous requirements are accomplish
    else{
        return(1);      // Received values are stored
    }    
}
void parse_HLSAT(const char* msg){
    int i = 0;
...
void write_buffer(volatile CircularBuffer* cb, char value){
    ...
    if (cb->writeIndex == cb->readIndex-1)
        cb->writeIndex;     // Dischargement of last value otherwise you are going 
...
} // INTERRUPT for reception from UART2

int read_buffer(volatile CircularBuffer* cb, char*  value){
    // Safe way to manage shared data
    // 1) disable the interrupt in reception 
    IEC1bits.U2RXIE = 0;
    if (cb->readIndex == cb->writeIndex){ //can't read and write in the same time
    ...
    while(read_buffer(&cb,&value) == 1){    // There is at least one unread value in the circular buffer
        if(parse_byte(&pstate,value) == NEW_MESSAGE){       // First value is a $ (new message is received)
            if(strcmp(pstate.msg_type,"HLREF") == 0){
                if(currentState != SAFE){
                    // Reset the counter of tmr2 in safe way
                    T2CONbits.TON = 0; // Disable the counter of tmr2
                    IEC0bits.T2IE = 1;  // Enable the interrupt of tmr2 (TIMEOUT)
...
void scheduler(){
    ...
    for (j = 1; j < MAX_TASKS+1; j++){
        schedInfo[j].n++;   // Increase the number of heartbeats that had elapsed
